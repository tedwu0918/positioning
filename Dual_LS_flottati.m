    ...
    while judge == 1
        ...
        if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
            ...
            elseif char( sat_sys( count ) ) == 66 %&& prn( count ) ~= 5 %&& ismember( prn(count) ,BD_sat ) == 1
                ...
                if obs_flag <= 2 || ( obs_flag == 3 && i <= initial_time )
                    Sat2s( prn(count)+32 , 2 ) = prn(count) + 100 ;      %   BD衛星加上100
...
%{
figure( 11 ) ;
plot( first_P+1:i , dx_Mtrix(first_P+1:i) ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'dx (m)') ;
%axis( [-1.8 , 1.8 ,-1.8 , 1.8 ] ) ;
title( 'dx隨時間之變化' ) ;
print( '-dpng',  'dx' , '-r600'  ) ;       %Change "-r600" to the required DPI
%}
