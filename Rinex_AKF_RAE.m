...
%[ CNRref ,CNRmean ,CNRstd ] =textread( '0710CSCref.csv' , '%d %f %f ' , 'delimiter' , ',' ) ;%headerlines:跳過第一行
%%
        ...
        if AnGsat ~=0
            ...
            if KF == 1 && i > initial_time % && any( Xxyz(i-1,:) ) == 1
                ...
                while norm( Delta_Xu(1:3) ) > 0.001
                    ...
                    Xustatic = Xpstatic + K * ( Z - (H*Xpstatic) ) ;        %   Z為修正過後的pseudorange
                    ...
                    Xpstatic = fystatic * Xustatic ;                  % fy為State transition matrix
            ...
            %rec_pos_01 = rec_pos1;          %   將所得之位置更新為下一時刻之初始位置
            %--------------將低仰角衛星之CSC資料清除----------
            %{
            if obs_flag > 1
                for q = 1 : size(stEL_G)
