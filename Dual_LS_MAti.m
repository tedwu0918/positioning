...
epk = 1 ;       %   已累積data之秒數
ac_lim = 30       %   累積data之秒數的上限 ( 固定秒數為(ac_lim-1),浮動秒數為1 )
interval = 240        %   累積data間的間隔時刻數
ma_Xxyz = [ ];
...
for i = 1 : runtime
    ...
    if nGsat ~= 0 && nBsat ~= 0
        ...
        while norm( Delta_Mtrix(1:3) ) > 0.01
            ...
            [ XS , dtS , XS_tx , VS_tx , time_tx , no_eph , sys_idx ] = ...                 %dtS已包含相對論誤差  %使用time_tx
                satellite_positions( r_gpst , Rr_G , GP_id , Eph_tatol , [] , [] , Tg , ig , rec_bias_satpos ) ;
            [ XS_B , dtS_B , XS_tx_B , VS_tx_B , time_tx_B , no_eph_B , sys_idx_B ] = ...   %使用dtS_B  XS_tx_B
        ...
        if nsat < 5
           ...
           ma_Xxyz = [ ma_Xxyz ; rec_pos ];    
           Xxyz(i,:) = [ mean( ma_Xxyz(  : , 1 ) ) mean( ma_Xxyz(  :  ,2) ) mean( ma_Xxyz (  : , 3 ) ) ];
           rec_pos_0 = Xxyz(i,:);
        end
        %---------------count DOP--------------------
