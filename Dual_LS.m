    ...
    while judge == 1
        ...
        if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
            ...
            if obs_flag <= 2 || ( obs_flag == 3 && i <= initial_time )
                ...
                runtime_GEL(GP_id(j,1),i)=EL_G( j , 1);
                j = j+1 ;
            ...
            elseif char( sat_sys( count ) ) == 66&&  prn( count ) ~= 11    &&  prn( count ) ~= 12%&&  prn( count ) ~= 5 
                ...
                if obs_flag <= 2 || ( obs_flag == 3 && i <= initial_time )
                    ...
                    runtime_BEL(BD_id(k,1),i)=EL_B( k , 1);
