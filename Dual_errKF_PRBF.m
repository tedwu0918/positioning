...
for i = 1 : runtime
    ...
    while judge == 1
        ...
        while sat_EL( count ) < smaller_than_elevation %|| cnr0( count ) < 30
            count = count + 1 ;
        end
        if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
            ...
            elseif char( sat_sys( count ) ) == 66 && prn( count ) ~= 5
                ...
                if obs_flag <= 2 || ( obs_flag == 3 && i <= initial_time )
                    Sat2s( prn(count)+32 , 2 ) = prn(count) + 100 ;      %   BD衛星+100
   ...
   %----------------每次(每一秒)定位衛星的顆數---------------
    ...
    if nGsat ~= 0 && nBsat ~= 0
       ...
       %-----------得到接收時間---------------------
        ...
        %-------------------------------------由衛星顆數判斷PBF之case-----------------------------------------
        if i > 2 && any( Xxyz(i-1,:) ) == 1 && any( Xxyz(i-2,:) ) == 1 && any( Sat2s(:,2) - Sat2s(:,1) ) ~= 0 %case:衛星顆數不變動(any:判斷矩陣內任一元素是否為非零,只要有非零值,傳回1)
