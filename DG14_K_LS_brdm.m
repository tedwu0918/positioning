...
switch Data
    ...
    case{ 0214 }
        ...
        GIM_filename = 'gpsg0450.17i' ;                                   % GIM全球電離層 ionex檔名
    ...
    for i = 1 : runtime
        ...
        while judge == 1
            ...
            if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
                if prn( count ) ~= 193  && all( AX( prn(count),: ) ) == 1 && AS( prn(count),1 ) ~= 999999.999999 && prn( count ) ~= 31
                    %|| prn( count ) ==  24  || prn( count ) ==  15 || prn( count ) ==  2)%&&prn( count ) ~= 24
