        ...
        if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
            ...
            elseif prn( count ) > 5000 && prn( count ) ~= 1193 && all( AX( prn(count)-5000,: ) ) == 1 && AS( prn(count)-5000,1 ) ~= 999999.999999%...
                ...
                if rem(ADR( count ),0.001) ==0
                    ...
                    cycle_slip_time_B(prn( count ),i) = 1 ;
                end
            end
            count = count + 1 ;
    ...
    if nGsat ~= 0 && nBsat ~= 0
        ...
        if AnGsat ~= 0 && AnBsat ~= 0
            ...
            if KF == 1 && i > initial_time % && any( Xxyz(i-1,:) ) == 1
                ...
                %--------------------Kalman Filter------------------------------
                [ Val , H ] = H_Mtrix( rec_pos_0 , Xs , AnGsat ,AnBsat , rec_bias_G , rec_bias_B ) ;
                Z = Rhoc - Val ;    % Z= z - zp
...
run_delta_B = run_ADR_B-run_trueRange_B;
%meanerror_B=mean(run_delta_B,2);
%run_delta_B = bsxfun(@minus , run_delta_B , meanerror_B ) ;
