...
switch Data
    ...
    case{ 01091 }
        ...
        GIM_filename = 'gpsg0090.17i' ;                                   % GIM全球電離層 ionex檔名
...
for i = 1 : runtime
    ...
    while judge == 1
        ...
        if time( count ) == count_times                                                       % all( )==1 , 即AX矩陣內元素都不為零
            if prn( count ) ~= 193  && all( AX( prn(count),: ) ) == 1 && AS( prn(count),1 ) ~= 999999.999999...
                    %&& (prn( count ) == 3 || prn( count ) == 14 || prn( count ) == 16 || prn( count ) == 25 || prn( count ) == 31 || prn( count ) == 22 || prn( count ) == 26)
    ...
    if nsat ~= 0
        ...
        elseif obs_flag == 3
            %[ Carrier , RMSE , mean_delta , ht ] = DCDRM_DG14( Carrier , PRr , lambda ,  RMSE , mean_delta , ht ) ;  %check cycle slip
...
%rec_pos_act = lla2ecef( [25.149442 , 121.777873 , 73.6] , 'WGS84' );  %海大
...
%{
figure( 11 ) ;
plot( TimeArray , runtime_error( 3 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP3' )
...
figure( 12 ) ;
plot( TimeArray , runtime_error( 14 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP14' )
...
figure( 13 ) ;
plot( TimeArray , runtime_error( 16 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP16' )
...
figure( 14 ) ;
plot( TimeArray , runtime_error( 22 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP22' )
...
figure( 15 ) ;
plot( TimeArray , runtime_error( 25 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP25' )
print( '-dpng',  'GP25-error' , '-r600'  ) ;

figure( 16 ) ;
plot( TimeArray , runtime_error( 26 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP26' )
...
figure( 17 ) ;
plot( TimeArray , runtime_error( 31 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP31' )
...
figure( 18 ) ;
plot( TimeArray , runtime_error( 32 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP32' )
...
figure( 20 ) ;
plot( TimeArray , runtime_error( 21 ,TimeArray ) , 'b-' ) ;
grid on ;
xlabel( 'Time (s)' ) ;
ylabel( 'error(m)' ) ;
title( 'GP21' )
