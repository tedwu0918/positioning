...
%%
% for R
Rhoerror = 3 ; Rrateerror = 0.01; 

%%
% for Q
sigma_p= 0.01 ;   sigma_b = 0.001 ;  
    ...
    if nsat ~= 0
        ...
        if AnGsat ~=0
            ...
            while norm( Delta_Mtrix(1:3) ) > 0.01
                ...
                lat_rec = llh_0( 1 ) * 180/pi ;          % unit : degree 緯
                lon_rec = llh_0( 2 ) * 180/pi ;         % unit : degree  經
            ...
            if KF == 1 && i > initial_time % && any( Xxyz(i-1,:) ) == 1
                ...
                %Rpr = diag((22.59*exp((-1)*CNR*0.04819)));%通用
                ...
                abs_Xp = xyz2llh( Xp([1,3,5])' ) ;
                lat = abs_Xp( 1 ) ;          %  緯
                lon = abs_Xp( 2 ) ;         % 經
                height = abs_Xp( 3 ) ;                  % unit : meter
                
                N=a/sqrt(1-e^2*sin(lat)^2);
                Z2=N*e^2*sin(lat);
                %gama = sqrt( (N+height)^2 + Z2^2 - 2*Z2*(N+height)*sin(lat) );
                gama = sqrt( Xp(1)^2 + Xp(3)^2 + Xp(5)^2 );
                
                A = gama\(  (N+height-Z2*sin(lat)) * (N^3*e^2*sin(lat)*cos(lat))/(a^2) +...
                    (Z2-(N+height)*sin(lat)) * (N*e^2*cos(lat)) * (1+(N^2*e^2*sin(lat)^2 )/(a^2)) -Z2*(N+height)*cos(lat) );
                B = (N^3*e^2*sin(lat)*cos(lat)^2)/(a^2)-(N+height)*sin(lat);
                KK = ( N + height -Z2*sin(lat) ) / gama - (A / B) * cos(lat) ;
                Rr = sqrt(Xp(1)^2 + Xp(3)^2 + Xp(5)^2);
                
                [ H ,Z , Val ] = H_outZ_single_prrate_full( Xs, Vs_rot, Xp([1,3,5],1)' , Rhoc_G ,   Ds  ,Ansat  );
                
                L = [ KK\( Xp(1)/Rr - (A/B)*cos(lon) )          0                                   KK\( Xp(3)/Rr - (A/B)*sin(lon) )                  0                     Xp(5)/(KK*Rr)                     0                0 0;
                            0                             KK\( Xp(1)/Rr - (A/B)*cos(lon) )                              0                            KK\( Xp(3)/Rr - (A/B)*sin(lon) )           0               Xp(5)/(KK*Rr)  0 0];
                ll=[(h0 - height );0];
                
                S = inv(Pp) + H'*inv(R)*H + L'*inv(D)*L ;             
                K = inv(S)*H'*inv(R);
                J =  inv(S)*L'*inv(D);               
                                
                Zp = H*Xp ;
                Xu = Xp + K* ( Z - Val ) - J*ll ;        %   Z為修正過後的pseudorange
                Vx = Xu - Xp ;
                
                I = eye( size(Xu,1)) ;
                Pu = (I - K*H +J*L) * Pp* (I - K*H +J*L)'+K*R*K' + J*D*J';
               % det(Pu)
              % [ Xu , Pu ] = KF_refine_full( Xu , Z ,Pu , R ,Xs ,Ansat,Vx);
            ...
            HHH = xyz2llh( rec_pos_0 ) ;
            
            h0_matrix = [h0_matrix ; HHH( 3 )] ;                  % unit : meter h0
            if i >=10
                %h0 = mean(h0_matrix);
                h0_matrix(1) = [];
            end
